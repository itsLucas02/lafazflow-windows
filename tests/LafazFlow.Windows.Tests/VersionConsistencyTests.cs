...
public sealed class VersionConsistencyTests
{
    ...
    public void ProjectInstallerPackageAndExpectedTagAgreeOnVersion()
    {
        ...
        Assert.Equal("1.1.2", projectVersion);
        Assert.Equal("1.1.2", informationalVersion);
        ...
        Assert.Equal("LafazFlow-1.1.2-win-x64-portable.zip", $"LafazFlow-{projectVersion}-win-x64-portable.zip");
        Assert.Equal("LafazFlow-1.1.2-setup.exe", $"LafazFlow-{projectVersion}-setup.exe");
        Assert.Equal("v1.1.2", $"v{projectVersion}");
    }
